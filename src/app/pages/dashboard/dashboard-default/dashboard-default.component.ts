...
export class DashboardDefaultComponent implements OnInit {
  ...
  realTimeConsumptionGraph(){
    this.electricData.getGroupOf().subscribe({
      next: (edata) => {
        ...
        this.data = temp2;
        console.log('Data del grafico', this.data)
        ...
        AmCharts.makeChart('past-bills', {
          ...
          dataProvider: this.pastMonthsBills
        });
  ...
  nilmGraph(){
    console.log("ASDAAAAS")
    let start = this.formatDate(this.nilmValues)
    let end = this.formatDate(this.getYesterday(this.nilmValues))

    this.electricData.getNilm(start, end).subscribe({
      next: (edata:any) => {
        console.log(edata)
  ...
  getInstantParameters(){
    this.electricData.getInstantParameters().subscribe({
      next: (edata:any) => {
        console.log(edata)
        const result = edata.reduce((acc, curr) => {
          acc[curr.type] = curr.data;
          return acc;
        }, {});
        this.voltaje_A = result.voltaje_A;
        this.voltaje_B = result.voltaje_B;
        this.voltaje_C = result.voltaje_C;
        this.corriente_A = result.corriente_A;
        this.corriente_B = result.corriente_B;
        this.corriente_C = result.corriente_C;
  ...
  getPastMonthsBills(){
    this.electricData.getPastMonths().subscribe({
      next: (data:any) => {
        const dataArray = Object.entries(data).map(([date, TotalConsumption]) => ({
          date,
          TotalConsumption
        }));
        
        console.log(dataArray);
        this.pastMonthsBills = dataArray;
